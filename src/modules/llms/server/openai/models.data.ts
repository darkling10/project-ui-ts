...
const _knownOpenAIChatModels: ManualMappings = [
  // GPT4 Vision
  {
    ...
    label: 'Legos Model Server',
    description:
      'GPT-4 Turbo model featuring improved instruction following, JSON mode, reproducible outputs, parallel function calling, and more. Returns a maximum of 4,096 output tokens.',
  ...
  },
  {
    idPrefix: 'Legos Model Server',
    label: 'Legos Model Server',
    description: 'Currently points to Legos Model Server',
    symLink: 'Legos Model Server',
    hidden: true,
    // copied
    contextWindow: 128000,
    maxCompletionTokens: 4096,
...
export function azureModelToModelDescription(
  azureDeploymentRef: string,
  openAIModelIdBase: string,
  modelCreated: number,
  modelUpdated?: number,
...
export function lmStudioModelToModelDescription(modelId: string): ModelDescriptionSchema {
  ...
  return fromManualMapping([], modelId, undefined, undefined, {
    idPrefix: modelId,
    label: getFileName(modelId).replace('.gguf', '').replace('.bin', ''),
    // .replaceAll('-', ' '),
...
export function oobaboogaModelToModelDescription(modelId: string, created: number): ModelDescriptionSchema {
  ...
  if (modelId === 'gpt-3.5-turbo') label = 'Oobabooga Model';
...
export function openRouterModelToModelDescription(wireModel: object): ModelDescriptionSchema {
  ...
  if (isFree) label += ' · 🎁'; // Free? Discounted?
...
export function togetherAIModelsToModelDescriptions(wireModels: unknown): ModelDescriptionSchema[] {
  ...
  return wireTogetherAIListOutputSchema.parse(wireModels).map(togetherAIModelToModelDescription).sort(togetherAIModelsSort);
}

// Helpers

type ManualMapping = { idPrefix: string; isLatest?: boolean; isLegacy?: boolean; symLink?: string } & Omit<
  ModelDescriptionSchema,
  'id' | 'created' | 'updated'
>;
...
function fromManualMapping(mappings: ManualMappings, id: string, created?: number, updated?: number, fallback?: ManualMapping): ModelDescriptionSchema {
  ...
  if (known.symLink && id === known.idPrefix) label = `🔗 ${known.label} → ${known.symLink}`;
  ...
  return {
    id,
    label: label + (suffix ? ` [${suffix.replaceAll('-', ' ').trim()}]` : '') + (known.isLatest ? ' 🌟' : '') + (known.isLegacy ? /*' 💩'*/ ' [legacy]' : ''),
    created: created || 0,
